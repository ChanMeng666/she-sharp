...
- **She Sharp is a registered charity.** A 2024 sponsorship draft described it
  ...
  legal name is "She Sharp". That name was live on this site as the `<h2>`
  directly above its own registration number, on the volunteer code of conduct,
  until 2026-09-01 (PR #258). It survives in `lib/data/` — 35 already-sent
  newsletter issues, the founder's speaker bios, one verbatim Humanitix payload
  — and is deliberately left there: those are historical records, not identity
  claims by the site.
- **The primary source is now archived**, so this no longer rests on a reading.
  The official Charity Summary PDF, generated by Charities Services 2026-09-01,
  is in the private archive repo at
  `governance/2026-09-01-charities-register/`, with the register page it came
  from. It confirms every row of the table above and records **no trading
  name** at all. `governance/README.md` there has the regeneration recipe.
