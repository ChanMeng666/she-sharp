...
### Resend Marketing objects — retired, not yet deleted
...
| Object | Name | ID | Env var | Status |
|---|---|---|---|---|
| Segment | `Newsletter` | `95d452f5-2eed-4ad4-b18e-5ff5a89a576b` | `RESEND_NEWSLETTER_SEGMENT_ID` | **Retired, pending deletion** |
| Segment | `General` (team default) | `9d195cb7-f7fc-49e0-9b88-e47c1741e720` | — | Never used |
| Topic | `Monthly Newsletter` (`opt_in`, private) | `08e59693-29dc-4556-8357-866dea047c6f` | `RESEND_NEWSLETTER_TOPIC_ID` | **Retired, pending deletion** |

**Nothing reads that segment or that topic for consent any more.** The subscription record is the `newsletter_subscribers` table (see "Monthly newsletter" below), and the newsletter now goes out through Resend's *transactional batch* API, which has no contact list, no segments and no topics. Do not wire anything new to these ids, and do not quote a subscriber count out of them — the count would be 0 and it would mean nothing. `lib/newsletter/resend-api.ts` and the two scripts that still import it (`scripts/newsletter/setup-resend.ts`, `scripts/newsletter/seed-pilot-contacts.ts`) belong to the same generation and go the same way; deleting the objects, the env vars and that code is a deliberate later step, not a tidy-up to do in passing.

**Both lists are empty.** Nothing was ever imported into the Resend team, and the subscriber table that replaced it holds no rows either. The real list — **1,560 subscribers** — is still in Mailchimp, and the live newsletter still goes out from there.
...
> - **Nothing has been sent and nobody has been imported.** The Resend list holds **0 contacts** — the account moved to the She Sharp–owned team on 2026-08-28 and not even a test address came across — and the `newsletter_subscribers` table that superseded it holds **0 rows**. Importing the 1,560 is the next phase, not a done one, and it keeps its plan block and its human approval. Anyone reading this after the code below shipped: the plumbing existing is not the list existing.
...
- **Logic in `lib/newsletter/`**: `assemble.ts` (builds `auto` from `lib/data/*`), `render.tsx` (React Email → HTML/text, `broadcast`/`preview` modes, 100KB Gmail-clip gate), `subscribers.ts` (the list itself — opt-in, confirm, exit, mailable reads), `generate.ts` (OpenAI editorial draft), `drafts.ts` (Redis staging), `schedule.ts` (last-Thursday send slot), `notify.ts` (Slack + admin-email review alerts). Email template in `emails/`. **`"broadcast"` here is a render mode, not a Resend broadcast** — it names the strict render that actually ships, as opposed to `"preview"`, and it kept its name through the move. `resend-api.ts` (typed REST wrapper over segments/topics/broadcasts) is **retired**: nothing in the send path imports it, only `setup-resend.ts` and `seed-pilot-contacts.ts` still do, and it goes when the Marketing objects above go.
...
- **Env** (see `.env.example`): `NEWSLETTER_ADMIN_EMAIL`, `SLACK_NEWSLETTER_WEBHOOK_URL` (falls back to `SLACK_CONTACT_WEBHOOK_URL`), `CRON_SECRET` (bearer for cron/admin triggers; must match Vercel), and `EMAIL_UNSUBSCRIBE_SECRET`, without which the batch builder refuses to write. Reuses `RESEND_API_KEY`, `DATABASE_URL` + the chatbot's Upstash Redis. `RESEND_NEWSLETTER_SEGMENT_ID` and `RESEND_NEWSLETTER_TOPIC_ID` are still set and still in `.env.example`, but **nothing in the send path reads them** — see the retired-objects table above before assuming otherwise.
