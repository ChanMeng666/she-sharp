...
- **The `subscribed` export was imported into `newsletter_subscribers`** by
  ...
  **That list was first sent to on 2026-08-31**, when the August 2026 newsletter
  went to all 1,549 mailable rows from Resend. **The July 2026 issue was the last
  newsletter ever sent from Mailchimp** — though the account is still live and
  its later campaigns in this archive (the Les Mills EDMs of 2026-08-18 and
  2026-08-22) are event mail, not newsletters.
...
until 2026-08-30 the site's only route to the back catalogue was
`MAILCHIMP_CONFIG.archiveUrl` — a live hyperlink that stops resolving the day
the account closes. That field, and `lib/data/newsletters.ts` with it, has since
been **deleted**; the 179 sent campaigns are committed to
`lib/data/newsletter-archive/` and served from `/resources/newsletters/<id>`,
with 428 objects re-hosted on Vercel Blob. The crosswalk below is what made that
possible, and it is still the measurement, not a belief: see the open-items table
and `docs/deployment/MAILCHIMP_CANCELLATION.md` §4.
`fetch-assets.ts` downloaded the gallery; what nobody had was
...
| The site's "Open full archive" button | **Closed 2026-08-30 by deleting it.** It was broken on a *paid* plan, and always had been, quite apart from the cancellation. `MAILCHIMP_CONFIG.archiveUrl` (`lib/data/newsletters.ts`) was rendered as "Open full archive" on `/resources/newsletters` and as "Read past issues" in the footer of every page. Fetched 2026-08-30 it returned HTTP 200 with exactly **20** `<li class="campaign">` entries across 17 dates, oldest **2026-02-14** — Mailchimp's archive page "shows links to the 20 most recent emails sent to your audience", so the ~180-campaign history the label implied had never been reachable from it on any plan. What was done: all **179** sent campaigns are committed to `lib/data/newsletter-archive/` and served from `/resources/newsletters/<id>`; the footer link now points at `/resources/newsletters`; the button was **removed rather than repointed**, because that page *is* the archive and the button would have linked it to itself. `MAILCHIMP_CONFIG.archiveUrl` was the last field in `lib/data/newsletters.ts`, so the file went with it. `scripts/mailchimp/archive-guard.test.ts` fails CI if a card is pointed back at Mailchimp. Detail: `docs/deployment/MAILCHIMP_CANCELLATION.md` §4 |
...
| The subscribe funnel | **Closed 2026-08-29.** Every site entry point now posts to `/api/newsletter/subscribe`, which writes a `pending` row to `newsletter_subscribers` — six of them on that date, and **eight inline forms since 2026-09-01** (`NEWSLETTER_SIGNUP_SURFACES.md`); `MAILCHIMP_CONFIG.subscribeUrl` was deleted rather than left as a second plausible destination. `EMAIL_AUTHENTICATION.md` item 8b. **Sending moved on 2026-08-31** — the August 2026 newsletter went from `newsletter_subscribers` through Resend to all 1,549, and July 2026 was Mailchimp's last newsletter. What has not moved is the account itself, which still sends event campaigns. The first send from `newsletter_subscribers` |
| **`campaigns.json` is stale by at least one send, and its `exportId` reads more current than its contents** | **Found 2026-08-30.** `metadata.exportId` says **`2026-08-27-api`** while `totals.lastSend` is **2026-08-22T00:45:00Z** over **180** campaigns — the pull date, not the data's currency, and the more recent-looking of the two. Ask `lastSend` how fresh this file is, never `exportId`. Against the live account: a read-only `GET /lists/31bd05e8eb` the same day returns `campaign_last_sent` **2026-08-27T20:37:33Z** and `campaign_count` **217**. So at least one campaign has gone out since the pull, and the archive is a snapshot rather than a live mirror. It will drift again with every Mailchimp send — the newsletter left on 2026-08-31, but the account still sends event campaigns from its console, so this file is not frozen. **Re-take before quoting a total:** `npx tsx scripts/mailchimp/build-campaigns.ts` against a fresh pull; check `metadata.exportId` and `totals.lastSend` before using any figure from it |
| **7 published newsletter URLs carried a per-contact `e=` parameter** | **Found and fixed 2026-08-30.** `lib/data/newsletters-archive.ts` held **7** `us3.campaign-archive.com` links ending `?e=<hash>` (for example `?e=20bef7c4a0`); `lib/data/newsletters-manual.ts` had **none** — checked, so that both files got looked at once and only one got edited. Mailchimp's `e=` is a **per-contact identifier**, how the hosted page knows which subscriber opened the link, so each was one real person's tracking token committed to a public repository. The parameter was stripped from all seven `source` values in place; history was **not** rewritten, because these are not addresses and not credentials. The `source` field now records the link the legacy site served, and `url` points at this site's own `/resources/newsletters/<id>` |
