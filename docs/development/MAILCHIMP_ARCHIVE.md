...
| **7 published newsletter URLs carry a per-contact `e=` parameter** | **Found 2026-08-30, not fixed here.** `lib/data/newsletters-archive.ts` holds **7** `us3.campaign-archive.com` links ending `?e=<hash>` (for example `?e=20bef7c4a0`). `lib/data/newsletters-manual.ts` has **none** — checked, so that both files get looked at once and only one gets edited. Mailchimp's `e=` is a **per-contact identifier** — it is how the hosted page knows which subscriber opened the link — so these are one real person's tracking token each, committed to a public GitHub repository. They are not addresses and they are not credentials, but they are per-person and they did not need to be there: the same page loads without the parameter. **Owned by whoever next touches that file**; strip the parameter, do not rewrite history for it |
