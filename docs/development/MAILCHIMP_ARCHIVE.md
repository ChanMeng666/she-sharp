...
| The subscribe funnel | **Closed 2026-08-29.** All six site entry points now post to `/api/newsletter/subscribe`, which writes a `pending` row to `newsletter_subscribers`; `MAILCHIMP_CONFIG.subscribeUrl` was deleted rather than left as a second plausible destination. `EMAIL_AUTHENTICATION.md` item 8b. **Sending moved on 2026-08-31** — the August 2026 newsletter went from `newsletter_subscribers` through Resend to all 1,549, and July 2026 was Mailchimp's last newsletter. What has not moved is the account itself, which still sends event campaigns. The first send from `newsletter_subscribers` |
