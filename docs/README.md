...
| `MAINTAINER_HANDOVER.md` | What stops working silently when the sole maintainer leaves — the access inventory, the three unwatched crons, the human obligations no script enforces, and the open loops that need a founder rather than a developer. §10 is the runbook for moving the production database off a personal account (Neon refuses to transfer a Vercel-managed project, so it is a copy, not a transfer); §11 records that Cloudinary is still live and still takes every profile photo. Written 2026-09-05 | current |
