...
| `newsletter/` | Monthly newsletter: photo strip, local preview, test send, approve. | `preview.ts` for review, `approve.ts` to ship. `refresh-pulse.ts` rebuilds **only** `editorial.pulse` against the live sources — the one part of `editorial` that is machine-produced — so the industry-news section can be brought up to date without regenerating the draft and taking the founder's note with it. Dry run by default; `--apply` writes. |
