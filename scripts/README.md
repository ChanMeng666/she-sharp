...
| --- | --- | --- |
| `clear-all-data.ts` | `TRUNCATE … RESTART IDENTITY CASCADE` over ~30 named tables, falling back to per-table `DELETE`. | **No `--dry-run`, no confirmation prompt, no environment guard.** It runs against whatever `DATABASE_URL` is loaded — on a machine that has pulled `.env` from Vercel that is production. Nothing is backed up first. |
| `reset-db-and-create-admin.ts` | Same `DELETE` sweep over ~25 tables, then inserts a hardcoded admin. | Same absent guards, plus it creates `admin@shesharp.org.nz` with the literal password `Admin123!` and a premium membership on whichever database it just wiped. |
...
| `renew-expired-invitations.ts` | For five people hardcoded in the source, marks old codes expired, mints new 30-day codes, and mails them. | Real send + DB write. The old code is dead once run. |
...
| `renew-expired-invitations.ts` | Renewed five named people's codes. | **One-off, 2026-04-14** — the five names are still in the source. |
...
- **`renew-expired-invitations.ts:44-54` hardcodes five real people's names and
  email addresses** in committed source.
