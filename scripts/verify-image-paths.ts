...
export const KNOWN_UNREFERENCED: Array<{ path: string; reason: string }> = [
  // SIX FILES WHOSE ONLY REFERENCE LEFT THE REPOSITORY.
  //
  // `report/` and `report-internal/` moved to NZ-SheSharp/she-sharp-reports on
  // 2026-09-01, and with them the `{ dir: "report" }` entry in
  // `scripts/assets/refs.ts` that made their manifests part of this corpus.
  // These six were referenced from there and nowhere else. They stay because
  // the funder report still places them — it now reads them across the
  // repository boundary through SHESHARP_REPO_DIR — so deleting them would
  // break that build silently, from over here, where nobody would look.
  //
  // This block and the removal of the scan root had to land in ONE commit:
  // adding these first makes them stale entries (still referenced), removing
  // the scan root first makes them orphans. Only both at once is green.
  //
  // Each was checked individually rather than swept in. Two of them are the
  // interesting ones: `group-photo.webp` looked referenced by
  // `lib/deck/wall-tiles.ts`, but that file names
  // `women-in-ai-for-social-good-group-photo.webp` — a different image whose
  // name merely ends the same way. And `Raquel.webp` is reachable in
  // `lib/data/team.ts` only from a COMMENTED-OUT member block, so the site has
  // not rendered it since she came off the team; the report still credits her.
  { path: "/img/events/she-sharp-candice-murray-own-your-energy/audience.webp", reason: "placed by the funder report (now NZ-SheSharp/she-sharp-reports, read via SHESHARP_REPO_DIR)" },
  { path: "/img/events/she-sharp-candice-murray-own-your-energy/volunteers.webp", reason: "placed by the funder report (now NZ-SheSharp/she-sharp-reports, read via SHESHARP_REPO_DIR)" },
  { path: "/img/events/she-sharp-and-academyex-international-womens-day-2026/group-photo.webp", reason: "placed by the funder report (now NZ-SheSharp/she-sharp-reports). NOT the same file as the similarly-named wall tile in lib/deck/wall-tiles.ts" },
  { path: "/img/team/Raquel.webp", reason: "credited in the funder report (now NZ-SheSharp/she-sharp-reports). Her lib/data/team.ts entry is commented out, so the site itself no longer renders it" },
  { path: "/img/sponsors/anz.svg", reason: "a past sponsor the funder report still lists (now NZ-SheSharp/she-sharp-reports); not in the current lib/data/sponsors.ts registry" },
  { path: "/img/sponsors/aws.svg", reason: "a past sponsor the funder report still lists (now NZ-SheSharp/she-sharp-reports); not in the current lib/data/sponsors.ts registry" },
  { path: "/img/sponsors/countdown.svg", reason: "a past sponsor the funder report still lists (now NZ-SheSharp/she-sharp-reports); not in the current lib/data/sponsors.ts registry" },
  { path: "/img/sponsors/google.svg", reason: "a past sponsor the funder report still lists (now NZ-SheSharp/she-sharp-reports); not in the current lib/data/sponsors.ts registry" },
  // Email-safe JPEG twin of aotearoa-ai-hackathon-festival-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  // them. The real guard for these files is
  // `npx tsx scripts/newsletter/email-covers.ts --check`, which fails when a
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/aotearoa-ai-hackathon-festival-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of code-secure-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/code-secure-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of event-lesmills-03-september-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/event-lesmills-03-september-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of her-waka-june-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/her-waka-june-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of peyvand-academy-13-june-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/peyvand-academy-13-june-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of peyvand-academy-20-june-2026's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/peyvand-academy-20-june-2026/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
  // Email-safe JPEG twin of she-sharp-and-myob-working-smarter's cover, generated by
  // `scripts/newsletter/email-covers.ts` and referenced by the newsletter issue
  // fixtures under lib/data/json/newsletter-issues/. It reads as unreferenced
  // only because scripts/assets/refs.ts deliberately keeps ABSOLUTE URLs out of
  // the corpus, and the `auto` block stores absolute URLs because email requires
  ...
  // referenced twin is missing — and which CI now runs, as a step in the
  // verify-image-paths job immediately after this one.
  { path: "/img/events/she-sharp-and-myob-working-smarter/cover-email.jpg", reason: "newsletter email-safe cover twin; referenced by an absolute URL in a newsletter issue fixture, which refs.ts excludes by design. Verified by scripts/newsletter/email-covers.ts --check, which CI runs in the verify-image-paths job" },
