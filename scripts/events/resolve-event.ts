...
// ---------------------------------------------------------------------------
// The shared resolution entry point
// ---------------------------------------------------------------------------

/** One scored near-miss, as `--json` reports it in the ambiguous case. */
...
/**
 * The three ways a query can end, mirroring the CLI's three exit codes:
 * `resolved` → 0, `no-match` → 1, `ambiguous` → 2.
 *
 * The shapes are the `--json` payloads verbatim, key order included, because
 * the CLI stringifies this value instead of building its own object. Anything
 * that changes here changes the JSON contract in the same commit — which is the
 * point: a caller reading the JSON and a caller importing the function must
 * never see two different answers about which event was meant.
 */
export type ResolveOutcome =
  | { mode: "resolved"; match: "exact-slug"; event: ResolvedEvent }
  | { mode: "resolved"; match: "fuzzy"; score: number; hits: string[]; event: ResolvedEvent }
  | { mode: "ambiguous"; query: string; candidates: ResolveCandidate[] }
  | { mode: "no-match"; query: string; candidates: never[] };

/**
 * Resolves a slug or half-remembered name to one event, or says why it cannot.
 *
 * An exact slug wins by construction — no scoring, no second guess. Otherwise
 * the query is tokenized and scored, and the leader must clear `MIN_SCORE` and
 * beat the runner-up by `CONFIDENT_RATIO` to be returned on its own; anything
 * closer comes back as `ambiguous` for a human to choose from, because picking
 * the wrong record is how a room number reaches the wrong hundred people.
 *
 * @param query Exact slug, or any words from the title/venue/city.
 * @param events Event records to search; defaults to the merged repo data.
 * @returns A discriminated outcome — resolved, ambiguous, or no-match.
 */
export function resolveEventByQuery(query: string, events: EventV3[] = getAllEvents()): ResolveOutcome {
  const exact = events.find((event) => event.slug === query);
  if (exact) return { mode: "resolved", match: "exact-slug", event: resolve(exact) };

  const tokens = tokenize(query);
  if (tokens.length === 0) return { mode: "no-match", query, candidates: [] };

  const ranked = rank(events.map((event) => score(event, tokens))).filter(
    (candidate) => candidate.score >= MIN_SCORE
  );

  if (ranked.length === 0) return { mode: "no-match", query, candidates: [] };
 ...
 */
function relativeWhen(event: ResolvedEvent): string {
  const time = parseDateString(event.date).getTime();
 ...
 *
 * Reads the resolved shape rather than the raw record so that both callers go
 * through the same flattening the `--json` output uses.
 */
function summarize(event: ResolvedEvent): string {
  const where = event.venueName || event.city || "location TBA";
...
/**
 * The CLI: parse the flags, ask `resolveEventByQuery()`, print the answer.
 *
 * Under `--json` it stringifies the outcome object as-is rather than assembling
 * its own — the exported type and the printed payload are then the same value,
 * so they cannot describe two different events.
 */
export async function main(): Promise<void> {
  ...
  switch (outcome.mode) {
    case "resolved": {
      if (!args.json) {
        if (outcome.match === "fuzzy") {
          console.log(
            `Matched "${args.query}" on ${outcome.hits.join(" + ")} (score ${outcome.score}).`
          );
          console.log("");
        }
        printResolved(outcome.event);
      }
      return;
    }
    case "no-match": {
      if (!args.json) printNoMatch(outcome.query);
      process.exit(1);
      break;
    }
    case "ambiguous": {
      if (!args.json) printCandidates(outcome.query, outcome.candidates);
...
// Run only when this file IS the command, so the announcement-spec generator
// can import `resolveEventByQuery()` without the CLI firing on import.
if (
  process.argv[1] &&
