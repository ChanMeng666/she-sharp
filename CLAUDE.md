...
- `app/robots.ts` — allows crawling, explicitly authorizes AI crawlers (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, …), advertises the sitemap. `DISALLOWED_PATHS` is exactly **`/dashboard/`, `/api/`, `/auth/`** — the sign-in/sign-up/password/verification pages were removed on 2026-08-09 and are kept out of search by `robots: { index: false }` on `app/(login)/layout.tsx` instead. See gotcha 4 below; do not put them back.
- `app/sitemap.ts` — static routes (in `STATIC_ROUTES`, incl. the mentor/mentee landing pages) + every event slug via `getAllEvents()` (**121 URLs** as of 2026-08-09 = 24 `STATIC_ROUTES` + 97 events; note 98 of them *look* like `/events/<slug>` because the hand-written `/events/google-educator-conference` series hub is a static route). The two `/mentorship/*/apply` routes are deliberately **not** in `STATIC_ROUTES`: they are `noindex`, and a noindex URL in the sitemap is the contradiction GSC reports as "Submitted URL marked 'noindex'". Do not re-add them when the registration window reopens. `/events/*/feedback` and `/f/*` are absent for the same reason — see the Event Feedback section.
  **Only past events carry `lastModified`.** Static routes and upcoming events emit none. They used to carry `new Date()`, i.e. the build timestamp, so every deploy told Google that 25 pages had just changed — and a lastmod that moves on every build is one Google learns to ignore site-wide, devaluing the entries that carry a real date. Omitting the field is honest; a fabricated date is not.
...
**Event titles carry their year.** `seoTitleFor()` in `app/(site)/events/[slug]/page.tsx` appends the event's year to the SERP title when the event's own name lacks one. Twelve years of recurring events means several share a name verbatim, and the site shipped **three** pages titled `International Women's Day | She Sharp` plus two each for `Google Educator Conference` and `She Celebrates` (found 2026-08-09). The **on-page name and `<h1>` are untouched** — those are the organisation's own record of what the event was called. Only the search-facing title is disambiguated, and it happens automatically for future collisions.

**Four metadata gotchas (do not regress):**
...
4. **A `Disallow` stops crawlers reading a `noindex`, so the two are alternatives, never a pair.** To keep a page out of search, make it crawlable and give it `noindex`. Blocking it in `robots.txt` instead means Google can still index the URL from external links while being unable to read the page — GSC calls that "Indexed, though blocked by robots.txt", and there is no way out until you unblock. This bit `/present/*` once and `/user-account` again on 2026-08-09, which 308'd into the robots-disallowed `/dashboard/` prefix and sat in the index with 27 search impressions. **Order matters when fixing it**: unblock first, then noindex — the reverse leaves the URL stuck.

**Verify after metadata edits**: run `npx tsx scripts/seo/verify-page-metadata.ts --base http://localhost:3100` against a local `next start`. It crawls every sitemap URL with a Googlebot UA and fails on a non-200, a missing or **duplicate** `<title>` or description, anything other than exactly one `<h1>`, a missing or non-self-referencing canonical, or a `noindex` on a sitemapped URL — the whole class of defect that only shows up when the pages are rendered together and compared. Not in CI (121 live requests tests the deployment, not the diff). Kill any orphan `next start` on the port first — a stale server silently serves an OLD build and makes changes look broken. For pages behind a date/feature gate, temporarily move the gate so the page actually renders; a redirecting page emits no metadata, so its `<head>` bugs stay invisible. Revert the gate before committing.

**`shesharp.org.nz` is more than this repo.** Two subdomains exist and neither is built here:
- `herwaka.shesharp.org.nz` — the HER WAKA programme docs, **Mintlify-hosted**, 82 public pages with their own sitemap and canonicals. Genuinely meant to be found.
- `hackathon.shesharp.org.nz` — the Aotearoa AI Hackathon Festival 2026 voice AI agent, a separate Next.js app on Vercel. Ships `noindex` deliberately.

This matters because a `sc-domain:` property covers **every subdomain**. On 2026-08-09, 91 of the 117 URLs in "Crawled – currently not indexed" were herwaka's `/mintlify-assets/*.js` chunks — 78% of a number that looked like a defect in this site. **Split any GSC figure by host before treating it as a bug here.**

**Search Console: three properties, and picking the wrong one is silently wrong.**
| Property | Use it for |
|---|---|
| `sc-domain:shesharp.org.nz` | DNS-verified root. **All issue history and every validation lives here** — this is the only property where VALIDATE FIX exists. |
| `https://www.shesharp.org.nz/` | This site's day-to-day reporting (added 2026-08-09, auto-verified off the domain property). |
| `https://herwaka.shesharp.org.nz/` | The docs site (added 2026-08-09). |

**Never compare counts across them** — the prefix properties exclude apex and other subdomains, worth ~3.5% of clicks and ~3.9% of impressions over the three months to 2026-08-09. A lower number there is a scope difference, not a drop.

**Access**: all three are reachable **only** from the `website@shesharp.org.nz` Google account, which lives in a *different Chrome instance* from the maintainer's usual profile. Guessing `authuser=1/2` URLs does not find it. With `claude-in-chrome`, use `switch_browser` (broadcasts a Connect prompt to every Chrome so the user picks) rather than `select_browser` — two separately-listed deviceIds both resolved to the same wrong profile.

**Exports**: the Page indexing report's own export carries **counts only**. The per-reason URL lists are a separate export per reason (click the row, then export from the drilldown, ≤1,000 rows each). Diagnosing anything here needs the drilldowns plus Links → internal links and Performance → Pages; the summary cannot distinguish a defect from a subdomain's static assets.

**Two conclusions worth not re-deriving** (both cost a full investigation on 2026-08-09):

1. **The ~90 event pages with zero internal links are not an indexing problem.** `/events` is a client component that server-renders only `EVENTS_PER_PAGE = 6` cards, so GSC's internal-links report sees just **16 target pages totalling 1,273 links** across the whole domain — the total matches exactly, so that is the full list, not a top-N. The orphaning is real. But 87 of 98 event pages draw impressions at a median position of **9.8** (vs 9.0 for non-event pages) and only 2 appear in "Crawled – currently not indexed": sitemap discovery is enough to index them. Restructuring `/events` is a defensible ranking/UX project (backlog item 10) — do **not** file it as a fix for an indexing bucket.
2. **Do not add `Disallow: /mintlify-assets/` to herwaka's robots.txt.** It looks like it would retire 91 entries; it would instead block the JavaScript Googlebot needs to render an 82-page site we want indexed. A `.js` resource sitting in "Crawled – currently not indexed" is the **normal** end state for a resource file, not an error. Rejected with reasoning as backlog item 12b.

**Docs** (in `docs/development/`): `GEO_SEO_IMPLEMENTATION_GUIDE.md` — reusable how-to (incl. browser-side GSC/Bing setup); `GEO_SEO_MONITORING.md` — KPIs, the 2026-06-23 baseline, the three properties, and the **running-validations table (check it before starting any validation — restarting a running one resets its progress)**; `GEO_SEO_BACKLOG.md` — prioritized follow-ups with live status, incl. the full 2026-08-09 drilldown in item 3c.
