...
- **Slack**: `sendEventFeedbackSlackNotification()` posts to `#event-feedback` via `SLACK_EVENT_FEEDBACK_WEBHOOK_URL`, falling back to `SLACK_CONTACT_WEBHOOK_URL`. The rating is in the message **header** so a week of feedback is scannable off the Slack message list without opening a card, and the `Feedback #<id>` context footer mirrors contact's `Submission #<id>` so tooling can join back to the row. Keep the two free-text answers in separate blocks clamped to 2,800 chars — Slack rejects the whole message above 3,000 per section, and the sender swallows the error, so the row would exist while the notification silently vanished.
