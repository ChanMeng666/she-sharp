 ...
 * and `lib/data/newsletters-manual.ts`; 51 of them were Mailchimp URLs in three
 * different shapes, written down over five years by whoever was sending that
 * month. Nothing in those files recorded a campaign id, so re-pointing a card at
 * an on-site render needs a join, and the join needs to be the same one every
 * time — hence one function here rather than a regex per call site.
 *
 ...
 *
 * The same resolver marks the Mailchimp cross-links *inside* the bodies, and
 * unpicks the 42 Facebook/Twitter share buttons that carry one percent-encoded
 * inside their query string — see `localiseArchivedHtml` in `./archive.ts`.
 *
 * WHY IT LIVES UNDER `lib/` and not beside the extractor that writes the index.
 * Three callers need it and they cannot share a `scripts/` module: the extractor
 * (Node, with the private vault), the CI guard (Node, no vault), and the Next.js
 * build that prerenders `/resources/newsletters/[issue]`. The root `tsconfig`
 * excludes `scripts/`, so a route importing from there would not typecheck and
 * would pull vault code into the bundle. `scripts/mailchimp/archive-index.ts`
 * re-exports everything here, so the extractor's imports are unchanged.
 *
