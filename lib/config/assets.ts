...
/**
 * Prefix for the newsletter back catalogue's re-hosted images.
 *
 * `lib/data/newsletter-archive/` holds 179 sent newsletters. Their images used
 * to be served from `mcusercontent.com`, which is the Mailchimp account that is
 * being cancelled — so saving the HTML without moving the pixels would have
 * hedged only half the risk. Each object is named for the **sha256 of its
 * source**, which makes the upload idempotent and, just as importantly, means no
 * original filename becomes a public URL: 39 of the source files name a person.
 *
 * There is deliberately no constant per image. 441 vault paths resolve to 428
 * objects, and the mapping that joins them is
 * `lib/data/newsletter-archive/images.json`, regenerated by
 * `scripts/mailchimp/rehost-archive-images.ts`. Five images are **withheld by
 * decision** and appear in neither the map nor the bodies —
 * `scripts/mailchimp/withheld-images.ts` says which and why.
 */
export const NEWSLETTER_ARCHIVE_IMAGE_PREFIX = `${BLOB_BASE}/newsletter-archive`;
