...
| **Step 7.2 — Frequency check** | `marketing-frequency-check.ts check --key announce-<slug>-<stage>` | **This is where a multi-stage campaign gets refused.** Three marketing sends a month across every skill, newsletter included; it exits non-zero. If the stages plus this month's newsletter come to four, the answer is normally to move a stage into the next month, not to override. **It counts only what this repo recorded** — it cannot see Mailchimp, which said `0/3` for an August 2026 in which the list took five marketing emails, so read the `NOT COUNTED` notice it prints and ask the marketing team before trusting a low number |
